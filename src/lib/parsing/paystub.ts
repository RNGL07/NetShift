...
export function parsePayStubText(text: string): { data: ParsedPayStub; confidence: ParseConfidence } {
  const lines = toTextLines(text);

  const data: ParsedPayStub = {
    payDate: normalizeDate(
      findLabeledValue(lines, ['pay date', 'check date', 'payment date', 'advice date', 'date of pay'], dateIn),
    ),
    periodStart: normalizeDate(
      findLabeledValue(lines, ['pay period start', 'period start', 'period beginning', 'from'], dateIn),
    ),
    periodEnd: normalizeDate(
      findLabeledValue(lines, ['pay period ending', 'period ending', 'pay period end', 'period end'], dateIn),
    ),
    ...
    regularHours: findLabeledValue(lines, ['regular hours', 'reg hours', 'reg hrs', 'straight time hours'], numberIn),
    overtimeHours: findLabeledValue(lines, ['overtime hours', 'ot hours', 'ot hrs', 'o/t hours'], numberIn),
...
export function scorePayStub(data: ParsedPayStub): ParseConfidence {
  ...
  const coreFound = [data.grossPay, data.netPay].filter((v) => v !== null && v !== undefined).length;
  ...
  if (coreFound === 2 && !sane) {
    reasons.push('The net pay found is larger than the gross pay, so a label matched the wrong number.');
  }
...
export const PAY_STUB_FIELDS: { key: keyof ParsedPayStub; label: string; type: 'text' | 'number' }[] = [
